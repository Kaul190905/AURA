...
class CaregiverService:
    ...
    async def invite_caregiver(self, owner_user_id: UUID, caregiver_email: str) -> CaregiverAssignment:
        ...
        return await self.caregiver_repo.create(owner_user_id, caregiver_id=caregiver_id, caregiver_email=caregiver_email)
    ...
    async def accept_invitation(self, caregiver_id: UUID, assignment_id: UUID) -> CaregiverAssignment:
        ...
        if not assignment:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Assignment not found.")
        if str(assignment.caregiver_id) != str(caregiver_id) and str(assignment.caregiver_id) != "None":
            # Caregiver ID mismatch and not null
        ...
        return await self.caregiver_repo.update(assignment_id, {"status": CaregiverStatus.ACTIVE, "caregiver_id": caregiver_id, "caregiver_email": None})
    ...
    async def get_pending_invitations(self, caregiver_id: UUID, caregiver_email: Optional[str] = None) -> List[CaregiverAssignment]:
        return await self.caregiver_repo.get_pending_invitations_for_caregiver(caregiver_id, caregiver_email)
