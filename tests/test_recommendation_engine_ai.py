...
def _make_response(text: str):
    """Fakes an anthropic Message response shape."""
    block = MagicMock()
    block.text = text
    response = MagicMock()
...
async def test_valid_llm_response_is_used(rule_engine):
    ...
    mock_client = AsyncMock()
    mock_client.messages.create.return_value = _make_response(json.dumps(personalized))
...
async def test_non_list_response_falls_back_to_rule_text(rule_engine):
    ...
    mock_client = AsyncMock()
    mock_client.messages.create.return_value = _make_response(json.dumps({"not": "a list"}))
...
async def test_malformed_json_falls_back_to_rule_text(rule_engine):
    ...
    mock_client = AsyncMock()
    mock_client.messages.create.return_value = _make_response("not valid json at all")
...
async def test_empty_string_items_fail_validation_and_fall_back(rule_engine):
    ...
    mock_client = AsyncMock()
    mock_client.messages.create.return_value = _make_response(json.dumps(["" for _ in eligible]))
